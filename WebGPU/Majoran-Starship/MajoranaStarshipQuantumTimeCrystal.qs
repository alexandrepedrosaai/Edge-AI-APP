...
namespace MajoranaStarship {
    ...
    operation MajoranaStarshipEngineQuantumTimeCrystal(input : Double[]) : Complex {
        ...
        for i in 0..19 {
            ...
            let brokenSymmetry = Complex(Microsoft.Quantum.Math.Exp(-d / 200.0), Microsoft.Quantum.Math.Log(1.0 + d)); // quebra de simetria temporal
            ...
            let decoherence = Complex(Microsoft.Quantum.Math.Exp(-d / 50.0), 0.0); // decoerência
            let energyGap = Complex(Sin(PI() * d / 70.0), Cos(PI() * d / 70.0)); // gap de energia
            let latticeTime = Complex(Microsoft.Quantum.Math.Log(1.0 + d), Microsoft.Quantum.Math.Exp(-d / 150.0)); // rede temporal discreta
            ...
            let recurrence = Complex(Microsoft.Quantum.Math.Exp(-d / 100.0), Microsoft.Quantum.Math.Log(1.0 + d)); // recorrência temporal
